...
function friendlyError(error: unknown): string {
  ...
  if (message.toLowerCase().includes('already been registered')) return 'Používateľ s týmto e-mailom už existuje.'
  return message || 'Pozvanie sa nepodarilo odoslať.'
}
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (auditError) console.warn('Audit pozvania sa nepodarilo zapísať:', auditError.message)
