-- IS Riadenie odboru v0.10.0
-- Supabase Auth, rozšírené profily, aplikačné roly, audit a bezpečné RPC.
-- Skript je idempotentný a je určený na spustenie po schema_v02.sql.
...
-- Pri vytvorení používateľa cez Dashboard alebo pozvánku vznikne základný profil.
...
-- Resolver môže ukladať prevádzkové zmeny spoločného snapshotu.
...
-- Profilová politika ostáva: čítanie v rámci organizácie, úpravy iba administrátorom.
...
select id, full_name, email, department, job_title, role, is_active, last_login_at
from public.profiles
order by full_name;
