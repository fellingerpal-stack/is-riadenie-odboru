...
function SupplierDetail({ item, canEdit, canOpenAdvanced, onEditSupplier, onDeleteSupplier, onNewRelationship, onEditRelationship, onConfirmRelationship, onRejectRelationship, onDeleteRelationship, go }: {
  ...
  return <article className="panel supplier-detail">
    <header className="supplier-detail-head">
      ...
      <div className="supplier-detail-actions"><button className="button button-secondary button-small" onClick={exportRelationships}><Icon name="download" size={15}/> CSV väzby</button>{canEdit && <><button className="button button-secondary button-small" onClick={onNewRelationship}><Icon name="plus" size={15}/> Väzba</button><button className="button button-secondary button-small" onClick={onEditSupplier}><Icon name="edit" size={15}/> Karta</button>{record && <button className="icon-button supplier-delete" onClick={onDeleteSupplier} title="Odstrániť manuálnu kartu"><Icon name="trash" size={17}/></button>}</>}</div>
    </header>
