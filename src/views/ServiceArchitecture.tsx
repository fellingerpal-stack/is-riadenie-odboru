...
export default function ServiceArchitecture({state,go,perspective}:{state:AppState;go:Go;perspective:Perspective}){
  ...
  return <>
    ...
    <div className="view-tabs architecture-tabs" role="tablist" aria-label="Architektonické pohľady"><button type="button" role="tab" aria-selected={tab==='map'} className={tab==='map'?'active':''} onClick={()=>setTab('map')}><Icon name="dashboard"/> Mapa služieb <b>{items.length}</b></button><button type="button" role="tab" aria-selected={tab==='dependencies'} className={tab==='dependencies'?'active':''} onClick={()=>setTab('dependencies')}><Icon name="substitute"/> Závislosti <b>{mapped.length}</b></button><button type="button" role="tab" aria-selected={tab==='locations'} className={tab==='locations'?'active':''} onClick={()=>setTab('locations')}><Icon name="database"/> Lokality <b>{locations.length}</b></button><button type="button" role="tab" aria-selected={tab==='gaps'} className={tab==='gaps'?'active':''} onClick={()=>setTab('gaps')}><Icon name="risk"/> Medzery <b>{gaps.length}</b></button></div>
