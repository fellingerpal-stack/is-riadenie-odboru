...
export default function InformationSystems({ canEdit, databaseMode, organizationId }: { canEdit: boolean; databaseMode: 'local' | 'cloud'; organizationId?: string }) {
  ...
  async function reload(silent = false) {
    ...
    try {
      setItems(await loadInformationSystems(databaseMode))
      ...
      if (databaseMode === 'cloud' && isDigitalPortfolioSchemaMissing(caught)) {
        const fallback = loadLocalInformationSystems()
        setItems(fallback)
        setSchemaFallback(true)
        setError(`Databázová tabuľka information_system_registry ešte nie je vytvorená. Dočasne zobrazujem ${fallback.length} zdrojových záznamov iba na čítanie. V Supabase spustite SQL opravu 0.13.1.`)
        setSync('error')
        return
      }
      const message = caught instanceof Error ? caught.message : 'Register informačných systémov sa nepodarilo načítať.'
  ...
  async function removeItem(item: InformationSystemRecord) {
    if (!effectiveCanEdit || !confirm(`Odstrániť informačný systém „${item.name}“ z registra?`)) return
  ...
  return <>
    <PageHeader eyebrow="Digitálne portfólio" title="Informačné systémy CVTI SR" description="Register systémov a softvéru s vecným a technickým vlastníctvom, kritickosťou, hostingom, SLA, zmluvami a ochranou údajov."
      actions={<><button className="button button-secondary" onClick={() => downloadCsv(filtered)}><Icon name="download" size={16}/> Export CSV</button>{effectiveCanEdit && <button className="button button-primary" onClick={() => { setDraft(blankSystem(items)); setModalTab('basic') }}><Icon name="plus" size={17}/> Nový IS</button>}</>}/>
    ...
    {draft && <Modal title={draft.name ? `Informačný systém: ${draft.name}` : 'Nový informačný systém'} onClose={() => setDraft(null)} wide><div className="portfolio-modal-tabs"><button className={modalTab === 'basic' ? 'active' : ''} onClick={() => setModalTab('basic')}>Základné údaje</button><button className={modalTab === 'owners' ? 'active' : ''} onClick={() => setModalTab('owners')}>Vlastníctvo a používatelia</button><button className={modalTab === 'contracts' ? 'active' : ''} onClick={() => setModalTab('contracts')}>SLA a zmluvy</button><button className={modalTab === 'security' ? 'active' : ''} onClick={() => setModalTab('security')}>Bezpečnosť a preverenie</button></div>
      {modalTab === 'basic' && <div className="form-grid"><Field label="Názov IS"><input disabled={!effectiveCanEdit} value={draft.name} onChange={(event) => update('name', event.target.value)}/></Field><Field label="Modul / oblasť"><input disabled={!effectiveCanEdit} value={draft.area} onChange={(event) => update('area', event.target.value)}/></Field><Field label="Stav prevádzky"><select disabled={!effectiveCanEdit} value={draft.operationStatus} onChange={(event) => update('operationStatus', event.target.value)}>{operationOptions.map((value) => <option key={value}>{value}</option>)}</select></Field><Field label="Kritickosť"><select disabled={!effectiveCanEdit} value={draft.criticality} onChange={(event) => update('criticality', event.target.value)}>{criticalityOptions.map((value) => <option key={value}>{value}</option>)}</select></Field><Field label="Hosting / prevádzka"><input disabled={!effectiveCanEdit} value={draft.hosting} onChange={(event) => update('hosting', event.target.value)}/></Field><Field label="Počet používateľov (odhad)"><input disabled={!effectiveCanEdit} value={draft.userCount} onChange={(event) => update('userCount', event.target.value)}/></Field><Field label="Využitie / účel IS"><textarea disabled={!effectiveCanEdit} value={draft.purpose} onChange={(event) => update('purpose', event.target.value)}/></Field></div>}
