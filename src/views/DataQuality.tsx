...
export default function DataQuality({state,canReadOit,canReadOris,canReadShared,go}:{state:AppState;canReadOit:boolean;canReadOris:boolean;canReadShared:boolean;go:(view:string)=>void}){
  const [severityFilter,setSeverityFilter]=useState<'all'|'danger'|'warning'|'info'>('all')
  ...
  const visibleSignals=severityFilter==='all'?signals:signals.filter(item=>item.severity===severityFilter)
  ...
  return <div className="data-quality-page"><PageHeader eyebrow="Data Quality Center" title="Kvalita a úplnosť dát" description="Jedna pracovná obrazovka pre chýbajúce vlastníctvo, identifikátory, väzby, kontinuitu a možné duplicity. Každý signál vedie priamo do modulu, kde sa dá opraviť." actions={<button className="button button-secondary" onClick={()=>go('myWorkspace')}><Icon name="dashboard" size={16}/>Moje centrum</button>}/>
    ...
    <div className="quality-kpis"><button className={severityFilter==='danger'?'active danger':''} onClick={()=>setSeverityFilter(value=>value==='danger'?'all':'danger')}><span>Kritické medzery</span><strong>{signals.filter(i=>i.severity==='danger').reduce((s,i)=>s+i.count,0)}</strong><small>klikni: vlastníctvo, duplicity, RACI</small></button><button className={severityFilter==='warning'?'active warning':''} onClick={()=>setSeverityFilter(value=>value==='warning'?'all':'warning')}><span>Na doplnenie</span><strong>{signals.filter(i=>i.severity==='warning').reduce((s,i)=>s+i.count,0)}</strong><small>klikni: kontinuita, inventúra, termíny</small></button><button className={severityFilter==='info'?'active info':''} onClick={()=>setSeverityFilter(value=>value==='info'?'all':'info')}><span>Informačné</span><strong>{signals.filter(i=>i.severity==='info').reduce((s,i)=>s+i.count,0)}</strong><small>klikni: väzby a profily</small></button><button className={severityFilter==='all'?'active':''} onClick={()=>setSeverityFilter('all')}><span>Typov kontrol</span><strong>{signals.length}</strong><small>zobraziť všetky relevantné kontroly</small></button></div>
    <section className="panel quality-panel"><div className="panel-heading"><div><span className="eyebrow">Opraviť</span><h3>Prioritizované medzery</h3></div><Badge tone={visibleSignals.some(i=>i.severity==='danger')?'danger':visibleSignals.length?'warning':'success'}>{visibleSignals.length} / {signals.length} kontrol</Badge></div><div className="quality-list">{visibleSignals.length?visibleSignals.map(signal=><button key={signal.id} className={`quality-row ${signal.severity}`} onClick={()=>go(signal.view)}><span className="quality-icon"><Icon name={signal.icon} size={18}/></span><span><strong>{signal.title}</strong><small>{signal.detail}</small></span><b>{signal.count}</b><span className="quality-open">Opraviť <Icon name="chevron" size={14}/></span></button>):<div className="quality-empty"><Icon name="check" size={28}/><strong>Bez signálov v tomto filtri</strong><span>Klikni na inú KPI kartu alebo zobraz všetky kontroly.</span></div>}</div></section>
  </div>
}
