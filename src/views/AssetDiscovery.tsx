...
function stringArray(value: unknown): string[] { return Array.isArray(value) ? value.map(text).filter(Boolean) : [] }
function classificationOf(device: DiscoveryDevice) { return objectValue(device.details.classification) }
function httpOf(device: DiscoveryDevice) { return Array.isArray(device.details.http) ? device.details.http.map(objectValue) : [] }
function dnsOf(device: DiscoveryDevice) { return objectValue(device.details.dns) }
function collectorConfig(projectUrl: string, collectorId: string) {
  return {
    supabase_url: projectUrl,
    supabase_anon_key_env: 'CVTI_SUPABASE_ANON_KEY',
    collector_id: collectorId,
    collector_token_env: 'CVTI_DISCOVERY_TOKEN',
    cidrs: ['10.0.0.0/24'],
    tcp_ports: [22,80,443,445,515,631,9100,3389],
    workers: 16,
    timeout_ms: 500,
    max_hosts_per_cidr: 4096,
    enrichment: { enabled:true, reverse_dns:true, http:true, https:true, tls:true, ssh_banner:true, local_neighbor_mac:true, classification:true, max_banner_bytes:4096, timeout_ms:800 },
    snmp: { enabled:false, probe_all:false, collect_supplies:true, community_env:'CVTI_SNMP_COMMUNITY', binary:'snmpget' },
  }
}
...
export default function AssetDiscovery({
  ...
  const [detailDevice, setDetailDevice] = useState<DiscoveryDevice | null>(null)
  ...
  const filtered = useMemo(() => {
    ...
    return devices.filter(device => {
      ...
      const classification = classificationOf(device)
      const dns = dnsOf(device)
      const http = httpOf(device)
      const haystack = `${device.ipAddress} ${device.macAddress} ${device.hostname} ${device.deviceType} ${device.manufacturer} ${device.model} ${device.serialNumber} ${device.firmware} ${text(classification.family)} ${text(classification.role)} ${text(classification.os_hint)} ${text(dns.ptr)} ${http.map(item=>`${text(item.title)} ${text(item.server)}`).join(' ')}`.toLowerCase()
  ...
  return <div className="discovery-page">
    ...
    {(view==='devices'||view==='print')&&<>
      ...
      <div className="table-shell discovery-table-shell"><table className="data-table discovery-table"><thead><tr><th>Zariadenie</th><th>Sieťová identita</th><th>Typ / scope</th><th>Discovery</th>{view==='print'&&<th>Print Fleet</th>}<th>Zhoda s registrom</th><th></th></tr></thead><tbody>{tableDevices.map(device=>{
        const currentStatus=deviceStatus(device); const match=matches.get(device.id)??null; const linked=items.find(item=>item.id===device.matchedCmdbId); const supplies=Array.isArray(device.details.supplies)?device.details.supplies as Record<string,unknown>[]:[]; const classification=classificationOf(device); const roleHint=text(classification.role); const confidence=numberValue(classification.confidence)
        return <tr key={device.id} className={currentStatus==='Nevidené'?'discovery-stale':''}><td><button className="discovery-device discovery-device-button" onClick={()=>setDetailDevice(device)}><span className="asset-ci-icon"><Icon name={view==='print'?'capacity':'cmdb'} size={17}/></span><span><strong>{device.hostname||device.model||device.ipAddress||'Neznáme zariadenie'}</strong><small>{[device.manufacturer,device.model,device.serialNumber&&`S/N ${device.serialNumber}`].filter(Boolean).join(' · ')||device.fingerprint.slice(0,12)}</small></span></button></td><td><strong>{device.ipAddress||'—'}</strong><small>{device.macAddress||'bez MAC'} · porty {device.openPorts.length?device.openPorts.join(', '):'—'}</small></td><td><strong>{device.deviceType}</strong><small>{scopeLabels[device.scope]}{device.firmware?` · FW ${device.firmware}`:''}</small>{roleHint&&<small className="discovery-role">{roleHint}{confidence?` · ${confidence}%`:''}</small>}</td><td><Badge tone={statusTone(currentStatus)}>{currentStatus}</Badge><small>first {formatDate(device.firstSeenAt)}<br/>last {formatDate(device.lastSeenAt)} · {device.seenCount}×</small>{device.changedFields.length>0&&<small className="discovery-changed">Δ {device.changedFields.join(', ')}</small>}</td>{view==='print'&&<td><strong>{numberValue(device.details.page_count)?`${numberValue(device.details.page_count).toLocaleString('sk-SK')} strán`:'bez počítadla'}</strong><div className="print-supplies">{supplies.slice(0,4).map((supply,index)=>{const pct=numberValue(supply.percent);return <span key={`${text(supply.name)}-${index}`} title={text(supply.name)}><i style={{width:`${Math.max(0,Math.min(100,pct))}%`}}/><b>{text(supply.name)||`Supply ${index+1}`} {pct?`${pct}%`:''}</b></span>})}</div></td>}<td>{linked?<button className="discovery-match linked" onClick={()=>onOpenAsset(linked)}><Icon name="check" size={14}/><span><strong>{linked.name}</strong><small>potvrdená väzba</small></span></button>:match?<div className="discovery-match"><span><strong>{match.asset.name}</strong><small>{match.confidence}% · {match.reason}</small></span>{canWriteScope(match.asset.scope)&&<button className="button button-secondary" disabled={busyId===device.id} onClick={()=>void linkToAsset(device,match.asset)}>Potvrdiť</button>}</div>:<span className="muted">bez kandidáta</span>}</td><td><div className="discovery-actions"><button className="icon-button" title="Device 360" onClick={()=>setDetailDevice(device)}><Icon name="eye" size={16}/></button>{!linked&&!match&&canWriteScope(device.scope)&&<button className="button button-primary" disabled={busyId===device.id} onClick={()=>void createAsset(device)}>Vytvoriť asset</button>}{!linked&&canWriteScope(device.scope)&&<button className="button button-ghost" disabled={busyId===device.id} onClick={()=>void ignore(device)}>{device.ignored?'Obnoviť':'Ignorovať'}</button>}{linked&&<button className="button button-secondary" onClick={()=>onOpenAsset(linked)}>Asset 360</button>}</div></td></tr>
    ...
    {detailDevice&&(()=>{const classification=classificationOf(detailDevice);const dns=dnsOf(detailDevice);const http=httpOf(detailDevice);const tls=objectValue(detailDevice.details.tls);const ssh=objectValue(detailDevice.details.ssh);const evidence=stringArray(classification.evidence);const linked=items.find(item=>item.id===detailDevice.matchedCmdbId);return <Modal wide title={`Device 360 · ${detailDevice.hostname||detailDevice.ipAddress||'Neznáme zariadenie'}`} onClose={()=>setDetailDevice(null)}><div className="discovery-device360"><section><span className="eyebrow">IDENTITA</span><dl><dt>Hostname</dt><dd>{detailDevice.hostname||'—'}</dd><dt>DNS / PTR</dt><dd>{text(dns.ptr)||'—'}</dd><dt>IP adresa</dt><dd>{detailDevice.ipAddress||'—'}</dd><dt>MAC</dt><dd>{detailDevice.macAddress||'—'}</dd><dt>Výrobca</dt><dd>{detailDevice.manufacturer||'—'}</dd><dt>Model</dt><dd>{detailDevice.model||'—'}</dd><dt>Sériové číslo</dt><dd>{detailDevice.serialNumber||'—'}</dd><dt>Firmware</dt><dd>{detailDevice.firmware||'—'}</dd></dl></section><section><span className="eyebrow">KLASIFIKÁCIA</span><div className="discovery-classification"><strong>{text(classification.family)||detailDevice.deviceType}</strong><Badge tone={numberValue(classification.confidence)>=85?'success':numberValue(classification.confidence)>=65?'warning':'neutral'}>{numberValue(classification.confidence)||0}% confidence</Badge><p>{text(classification.role)||'Bez odvodeného rolu'}</p><small>{text(classification.os_hint)||'OS zatiaľ neurčený'}</small></div><div className="discovery-evidence">{evidence.map(item=><span key={item}>{item}</span>)}</div></section><section><span className="eyebrow">SIEŤ A SLUŽBY</span><dl><dt>Porty</dt><dd>{detailDevice.openPorts.join(', ')||'—'}</dd><dt>HTTP/HTTPS</dt><dd>{http.length?http.map(item=>`${text(item.scheme).toUpperCase()} ${numberValue(item.status)||''} ${text(item.title)||text(item.server)}`).join(' · '):'—'}</dd><dt>TLS</dt><dd>{text(tls.protocol)?`${text(tls.protocol)} · ${text(tls.cipher)}`:'—'}</dd><dt>SSH</dt><dd>{text(ssh.banner)||'—'}</dd></dl></section><section><span className="eyebrow">DISCOVERY</span><dl><dt>Scope</dt><dd>{scopeLabels[detailDevice.scope]}</dd><dt>First seen</dt><dd>{formatDate(detailDevice.firstSeenAt)}</dd><dt>Last seen</dt><dd>{formatDate(detailDevice.lastSeenAt)}</dd><dt>Počet pozorovaní</dt><dd>{detailDevice.seenCount}×</dd><dt>Collector</dt><dd>{collectorMap.get(detailDevice.lastCollectorId)||detailDevice.lastCollectorId||'—'}</dd><dt>Stav</dt><dd>{deviceStatus(detailDevice)}</dd></dl></section>{linked&&<section className="discovery-device360-wide"><span className="eyebrow">ASSET 360</span><div className="discovery-linked-asset"><div><strong>{linked.name}</strong><small>{linked.assetTag||linked.hostname||linked.id}</small></div><button className="button button-primary" onClick={()=>onOpenAsset(linked)}>Otvoriť Asset 360</button></div></section>}</div></Modal>})()}

    {secret&&<Modal wide title="Collector token · zobrazí sa iba teraz" onClose={()=>setSecret(null)}><div className="collector-secret"><div className="inline-alert inline-alert-warning"><Icon name="warning" size={17}/><span>Token ulož do bezpečného secret store. Po zatvorení ho aplikácia už nevie zobraziť; vie iba vygenerovať nový.</span></div><Field label="Collector ID"><input readOnly value={secret.collector.id}/></Field><Field label="Collector token"><textarea readOnly rows={3} value={secret.token}/></Field><div className="collector-config"><strong>Config collectora v0.31.2</strong><pre>{JSON.stringify(collectorConfig(projectUrl,secret.collector.id),null,2)}</pre></div><div className="modal-actions"><button className="button button-secondary" onClick={()=>downloadText(`collector-${secret.collector.name.replace(/[^a-z0-9]+/gi,'-').toLowerCase()}.json`,JSON.stringify(collectorConfig(projectUrl,secret.collector.id),null,2))}><Icon name="download" size={15}/>Stiahnuť config</button><button className="button button-primary" onClick={()=>setSecret(null)}>Hotovo</button></div></div></Modal>}
  </div>
}
