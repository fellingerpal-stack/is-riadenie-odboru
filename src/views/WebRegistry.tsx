...
import {
  ...
  loadWebsites,
  loadLocalWebsites,
  isDigitalPortfolioSchemaMissing,
  normalizeWebsite,
  saveLocalWebsites,
...
export default function WebRegistry({ canEdit, databaseMode, organizationId }: { canEdit: boolean; databaseMode: 'local' | 'cloud'; organizationId?: string }) {
  ...
  async function reload(silent = false) {
    ...
    try {
      setItems(await loadWebsites(databaseMode))
      setSchemaFallback(false)
      setSync(databaseMode === 'cloud' ? 'synced' : 'local')
    } catch (caught) {
      if (databaseMode === 'cloud' && isDigitalPortfolioSchemaMissing(caught)) {
        const fallback = loadLocalWebsites()
        setItems(fallback)
        setSchemaFallback(true)
        setError(`Databázová tabuľka website_registry ešte nie je vytvorená. Dočasne zobrazujem ${fallback.length} zdrojových záznamov iba na čítanie. V Supabase spustite SQL opravu 0.13.1.`)
        setSync('error')
        return
      }
      const message = caught instanceof Error ? caught.message : 'Register webov sa nepodarilo načítať.'
      setSchemaFallback(false); setError(message); setSync('error'); if (!silent) alert(message)
    }
  }
  ...
  async function removeItem(item: WebsiteRecord) {
    if (!effectiveCanEdit || !confirm(`Odstrániť web „${item.name}“ z registra?`)) return
  ...
  return <>
    <PageHeader eyebrow="Digitálne portfólio" title="Weby CVTI SR" description="Centrálny register webov, technických údajov, vlastníctva, duplicít a rozhodnutí z inventarizácie WS02 a weby_IS."
      actions={<><button className="button button-secondary" onClick={() => downloadCsv(filtered)}><Icon name="download" size={16}/> Export CSV</button>{effectiveCanEdit && <button className="button button-primary" onClick={() => { setDraft(blankWebsite(items)); setModalTab('basic') }}><Icon name="plus" size={17}/> Nový web</button>}</>}/>
    ...
    {draft && <Modal title={draft.name ? `Web: ${draft.name}` : 'Nový web CVTI SR'} onClose={() => setDraft(null)} wide><div className="portfolio-modal-tabs"><button className={modalTab === 'basic' ? 'active' : ''} onClick={() => setModalTab('basic')}>Základné údaje</button><button className={modalTab === 'technical' ? 'active' : ''} onClick={() => setModalTab('technical')}>Technika</button><button className={modalTab === 'content' ? 'active' : ''} onClick={() => setModalTab('content')}>Obsah a vlastníctvo</button><button className={modalTab === 'decision' ? 'active' : ''} onClick={() => setModalTab('decision')}>Rozhodnutie a riziká</button></div>
      {modalTab === 'basic' && <div className="form-grid"><Field label="Názov webu"><input disabled={!effectiveCanEdit} value={draft.name} onChange={(event) => update('name', event.target.value)}/></Field><Field label="URL"><input disabled={!effectiveCanEdit} value={draft.url} onChange={(event) => update('url', event.target.value)}/></Field><Field label="Normalizovaná doména"><input disabled={!effectiveCanEdit} value={draft.normalizedDomain} onChange={(event) => update('normalizedDomain', event.target.value)}/></Field><Field label="Stav porovnania"><select disabled={!effectiveCanEdit} value={draft.comparisonStatus} onChange={(event) => update('comparisonStatus', event.target.value)}>{comparisonOptions.map((value) => <option key={value}>{value}</option>)}</select></Field><Field label="Istota zhody"><input disabled={!effectiveCanEdit} value={draft.matchConfidence} onChange={(event) => update('matchConfidence', event.target.value)}/></Field><Field label="Dátum stretnutia"><input disabled={!effectiveCanEdit} type="date" value={draft.meetingDate} onChange={(event) => update('meetingDate', event.target.value)}/></Field><Field label="Aktívny a relevantný?"><input disabled={!effectiveCanEdit} value={draft.activeRelevant} onChange={(event) => update('activeRelevant', event.target.value)}/></Field><Field label="EÚ financovanie – online?"><input disabled={!effectiveCanEdit} value={draft.euFundedOnline} onChange={(event) => update('euFundedOnline', event.target.value)}/></Field><Field label="EÚ financovanie – do kedy?"><input disabled={!effectiveCanEdit} value={draft.euFundedUntil} onChange={(event) => update('euFundedUntil', event.target.value)}/></Field><Field label="Prekrývajúci web"><input disabled={!effectiveCanEdit} value={draft.overlappingSite} onChange={(event) => update('overlappingSite', event.target.value)}/></Field></div>}
      {modalTab === 'technical' && <div className="form-grid"><Field label="DNS názvy / aliasy"><input disabled={!effectiveCanEdit} value={draft.dnsAliases} onChange={(event) => update('dnsAliases', event.target.value)}/></Field><Field label="Verejná IP"><input disabled={!effectiveCanEdit} value={draft.publicIp} onChange={(event) => update('publicIp', event.target.value)}/></Field><Field label="LB VIP"><input disabled={!effectiveCanEdit} value={draft.lbVip} onChange={(event) => update('lbVip', event.target.value)}/></Field><Field label="Server IP"><input disabled={!effectiveCanEdit} value={draft.serverIp} onChange={(event) => update('serverIp', event.target.value)}/></Field><Field label="Sekcia weby_IS"><input disabled={!effectiveCanEdit} value={draft.technicalSection} onChange={(event) => update('technicalSection', event.target.value)}/></Field><Field label="Technický vlastník"><input disabled={!effectiveCanEdit} value={draft.technicalOwner} onChange={(event) => update('technicalOwner', event.target.value)}/></Field><Field label="Platforma"><input disabled={!effectiveCanEdit} value={draft.platform} onChange={(event) => update('platform', event.target.value)}/></Field><Field label="Vlastný brand manuál?"><input disabled={!effectiveCanEdit} value={draft.hasOwnBrandManual} onChange={(event) => update('hasOwnBrandManual', event.target.value)}/></Field><Field label="Brand manuál dostupný?"><input disabled={!effectiveCanEdit} value={draft.brandManualAvailable} onChange={(event) => update('brandManualAvailable', event.target.value)}/></Field><Field label="Technický komentár"><textarea disabled={!effectiveCanEdit} value={draft.technicalComment} onChange={(event) => update('technicalComment', event.target.value)}/></Field><Field label="Interaktívne funkcie?"><textarea disabled={!effectiveCanEdit} value={draft.interactiveFunctions} onChange={(event) => update('interactiveFunctions', event.target.value)}/></Field><Field label="Funkcie / napojenia"><textarea disabled={!effectiveCanEdit} value={draft.integrationDescription} onChange={(event) => update('integrationDescription', event.target.value)}/></Field></div>}
      {modalTab === 'content' && <div className="form-grid"><Field label="Garant / kontakt"><input disabled={!effectiveCanEdit} value={draft.businessContact} onChange={(event) => update('businessContact', event.target.value)}/></Field><Field label="Sekcia / oddelenie"><input disabled={!effectiveCanEdit} value={draft.businessUnit} onChange={(event) => update('businessUnit', event.target.value)}/></Field><Field label="Zodpovedný za obsah"><input disabled={!effectiveCanEdit} value={draft.contentOwner} onChange={(event) => update('contentOwner', event.target.value)}/></Field><Field label="Frekvencia aktualizácie"><input disabled={!effectiveCanEdit} value={draft.updateFrequency} onChange={(event) => update('updateFrequency', event.target.value)}/></Field><Field label="Primárny účel webu"><textarea disabled={!effectiveCanEdit} value={draft.primaryPurpose} onChange={(event) => update('primaryPurpose', event.target.value)}/></Field><Field label="Cieľová skupina"><textarea disabled={!effectiveCanEdit} value={draft.targetAudience} onChange={(event) => update('targetAudience', event.target.value)}/></Field><Field label="Typy obsahu"><textarea disabled={!effectiveCanEdit} value={draft.contentTypes} onChange={(event) => update('contentTypes', event.target.value)}/></Field><Field label="Najdôležitejšie stránky"><textarea disabled={!effectiveCanEdit} value={draft.keyPages} onChange={(event) => update('keyPages', event.target.value)}/></Field><Field label="Rôzne používateľské skupiny?"><input disabled={!effectiveCanEdit} value={draft.differentUserGroups} onChange={(event) => update('differentUserGroups', event.target.value)}/></Field><Field label="Plán rozvoja"><textarea disabled={!effectiveCanEdit} value={draft.developmentPlan} onChange={(event) => update('developmentPlan', event.target.value)}/></Field></div>}
      {modalTab === 'decision' && <div className="form-grid"><Field label="Odporúčané opatrenie"><select disabled={!effectiveCanEdit} value={draft.recommendedAction} onChange={(event) => update('recommendedAction', event.target.value)}>{actionOptions.map((value) => <option key={value}>{value}</option>)}</select></Field><Field label="Stav kontroly"><select disabled={!effectiveCanEdit} value={draft.reviewStatus} onChange={(event) => update('reviewStatus', event.target.value)}>{reviewOptions.map((value) => <option key={value}>{value}</option>)}</select></Field><Field label="Skupina duplicity"><input disabled={!effectiveCanEdit} value={draft.duplicateGroup} onChange={(event) => update('duplicateGroup', event.target.value)}/></Field><Field label="Typ duplicity / vzťahu"><input disabled={!effectiveCanEdit} value={draft.duplicateType} onChange={(event) => update('duplicateType', event.target.value)}/></Field><Field label="Navrhovaný kanonický záznam"><input disabled={!effectiveCanEdit} value={draft.canonicalRecord} onChange={(event) => update('canonicalRecord', event.target.value)}/></Field><Field label="Priorita"><input disabled={!effectiveCanEdit} value={draft.priority} onChange={(event) => update('priority', event.target.value)}/></Field><Field label="Najväčší problém webu"><textarea disabled={!effectiveCanEdit} value={draft.largestProblem} onChange={(event) => update('largestProblem', event.target.value)}/></Field><Field label="Čo by zmenili"><textarea disabled={!effectiveCanEdit} value={draft.desiredChange} onChange={(event) => update('desiredChange', event.target.value)}/></Field><Field label="Definícia úspechu"><textarea disabled={!effectiveCanEdit} value={draft.successDefinition} onChange={(event) => update('successDefinition', event.target.value)}/></Field><Field label="Poznámka k rozhodnutiu"><textarea disabled={!effectiveCanEdit} value={draft.decisionNote} onChange={(event) => update('decisionNote', event.target.value)}/></Field><Field label="Kategória analytika"><input disabled={!effectiveCanEdit} value={draft.analystCategory} onChange={(event) => update('analystCategory', event.target.value)}/></Field><Field label="Odôvodnenie kategórie"><textarea disabled={!effectiveCanEdit} value={draft.analystReason} onChange={(event) => update('analystReason', event.target.value)}/></Field><Field label="Poznámky analytika"><textarea disabled={!effectiveCanEdit} value={draft.analystNotes} onChange={(event) => update('analystNotes', event.target.value)}/></Field></div>}
      <div className="portfolio-source-note"><Icon name="database" size={17}/><span><strong>Zdrojový záznam</strong>{draft.sourceFile || 'Aplikácia'} · {draft.sourceSheet || 'Weby CVTI SR'} · riadok {draft.sourceRow || 'nový'}</span></div><div className="modal-actions"><button className="button button-secondary" onClick={() => setDraft(null)}>Zavrieť</button>{effectiveCanEdit && <button className="button button-primary" onClick={() => void saveDraft()} disabled={!draft.name.trim()}><Icon name="check" size={16}/> Uložiť web</button>}</div>
    </Modal>}
  </>
}
