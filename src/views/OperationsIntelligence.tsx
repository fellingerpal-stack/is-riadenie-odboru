...
export default function OperationsIntelligence({state,go}:{state:AppState;go:Go}){
  ...
  return <div className="ops-intelligence">
    ...
    {tab==='services'&&<section className="ops-service-workspace">
      ...
      {selectedService?<ServiceDetail record={selectedService} go={go}/>:<div className="panel"><Empty title="Služba nebola vybraná" text="Vyberte službu v ľavom zozname."/></div>}
    </section>}
    ...
    {tab==='vendors'&&<VendorView task={vendorTask} setTask={setVendorTask}/>} 
    {tab==='forecast'&&<ForecastView task={forecastTask} setTask={setForecastTask} method={forecastMethod} setMethod={setForecastMethod}/>} 
  </div>
}

function ServiceDetail({record,go}:{record:ReturnType<typeof buildService360>[number];go:Go}){
...
function VendorView({task,setTask}:{task:string;setTask:(value:string)=>void}){
  ...
  return <>
    ...
    <section className="panel ops-vendor-panel"><div className="panel-heading"><div><span className="eyebrow">VENDOR DEPENDENCY</span><h3>Koncentrácia platieb podľa firmy/IČO</h3><p>Názov dodávateľa sa nevymýšľa. Ak zdroj obsahuje iba identifikátor firmy/IČO, zobrazí sa identifikátor. Koncentrácia je finančná stopa, nie hodnotenie dodávateľa.</p></div></div><div className="ops-vendor-list">{rows.map((row,index)=><article key={`${row.task}-${row.supplierId}`}><div className="ops-vendor-rank">{index+1}</div><span className="ops-vendor-name"><strong>{row.supplierLabel}</strong><small>Úloha {row.task} · {row.paymentCount} platieb · strediská {row.centers.join(', ')||'—'}</small></span><div className="ops-vendor-bar"><i style={{width:`${row.amount/max*100}%`}}/></div><strong className="ops-vendor-amount">{money.format(row.amount)}</strong><Badge tone={row.mapping.some(value=>value.includes('reconciliation'))?'warning':'success'}>{row.mapping.some(value=>value.includes('reconciliation'))?'pravidlo':'priame'}</Badge><details><summary>Zmluvy a poznámky</summary><p><b>Zmluvy:</b> {row.contracts.join(', ')||'bez referencie'}</p><p><b>Typické položky:</b> {row.topNotes.join(' · ')||'—'}</p></details></article>)}</div></section>
