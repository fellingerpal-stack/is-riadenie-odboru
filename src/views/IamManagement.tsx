...
export default function IamManagement({ accessRequests, accessCatalog, recertificationCampaigns, services, employees, tasks, canEdit, currentUser, onAccessRequestsChange, onAccessCatalogChange, onRecertificationCampaignsChange, onTasksChange }: Props) {
  ...
  function openNewCatalog() { setCatalogDraft(blankCatalog(accessCatalog)); setCatalogModal(true) }
  function openCatalog(item: AccessCatalogItem) { setCatalogDraft(structuredClone(item)); setCatalogModal(true) }
  function saveCatalog() {
    if (!catalogDraft.name.trim()) return
  ...
  return <div className="iam-page">
    ...
    {view === 'catalog' && <><div className="iam-section-head"><div><span className="eyebrow">Katalóg oprávnení</span><h2>Štandardizované prístupy</h2><p>Každá položka určuje vlastníka, riziko, platnosť a schvaľovaciu cestu.</p></div><button className="button button-primary" onClick={openNewCatalog} disabled={!canEdit}><Icon name="plus"/> Nová položka</button></div><div className="iam-catalog-grid">{accessCatalog.map((item) => { const service = services.find((entry) => entry.id === item.serviceId); return <button key={item.id} onClick={() => openCatalog(item)} className={!item.isActive ? 'is-inactive' : ''}><header><span className="iam-catalog-icon"><Icon name={item.privileged ? 'lock' : 'services'}/></span><div><strong>{item.name}</strong><small>{item.id} · {item.system}</small></div><Badge tone={riskTone(item.risk)}>{item.risk}</Badge></header><p>{item.description}</p><dl><div><dt>Služba</dt><dd>{service?.name || 'Neurčená'}</dd></div><div><dt>Vlastník</dt><dd>{item.businessOwner || 'Neurčený'}</dd></div><div><dt>Platnosť</dt><dd>{item.defaultDurationDays ? `${item.defaultDurationDays} dní` : 'Bez obmedzenia'}</dd></div><div><dt>Schválenie</dt><dd>{item.approvalPath.join(' → ')}</dd></div></dl></button>})}</div></>}
    ...
    {catalogModal && <Modal title={`${catalogDraft.id} · Katalógová položka`} onClose={() => setCatalogModal(false)}><div className="iam-catalog-form"><Field label="Názov"><input value={catalogDraft.name} onChange={(event) => setCatalogDraft({ ...catalogDraft, name: event.target.value })}/></Field><Field label="Systém / rola"><input value={catalogDraft.system} onChange={(event) => setCatalogDraft({ ...catalogDraft, system: event.target.value })}/></Field><Field label="Služba"><select value={catalogDraft.serviceId} onChange={(event) => setCatalogDraft({ ...catalogDraft, serviceId: event.target.value })}><option value="">Neurčená</option>{services.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></Field><Field label="Riziko"><select value={catalogDraft.risk} onChange={(event) => setCatalogDraft({ ...catalogDraft, risk: event.target.value })}>{risks.map((item) => <option key={item}>{item}</option>)}</select></Field><Field label="Biznis vlastník"><input value={catalogDraft.businessOwner} onChange={(event) => setCatalogDraft({ ...catalogDraft, businessOwner: event.target.value })}/></Field><Field label="Technický vlastník"><input value={catalogDraft.technicalOwner} onChange={(event) => setCatalogDraft({ ...catalogDraft, technicalOwner: event.target.value })}/></Field><Field label="Predvolená platnosť (dni)"><input type="number" min="0" value={catalogDraft.defaultDurationDays} onChange={(event) => setCatalogDraft({ ...catalogDraft, defaultDurationDays: Number(event.target.value) })}/></Field><label className="iam-checkbox"><input type="checkbox" checked={catalogDraft.privileged} onChange={(event) => setCatalogDraft({ ...catalogDraft, privileged: event.target.checked })}/> Privilegovaný prístup</label><label className="iam-checkbox"><input type="checkbox" checked={catalogDraft.isActive} onChange={(event) => setCatalogDraft({ ...catalogDraft, isActive: event.target.checked })}/> Aktívna položka</label><Field label="Opis"><textarea value={catalogDraft.description} onChange={(event) => setCatalogDraft({ ...catalogDraft, description: event.target.value })}/></Field></div><div className="modal-actions split-actions"><div>{accessCatalog.some((item) => item.id === catalogDraft.id) && <button className="button button-danger" onClick={deleteCatalog}><Icon name="trash"/> Odstrániť</button>}</div><div><button className="button button-secondary" onClick={() => setCatalogModal(false)}>Zrušiť</button><button className="button button-primary" onClick={saveCatalog} disabled={!catalogDraft.name.trim()}>Uložiť</button></div></div></Modal>}

    {campaignModal && campaignDraft && <Modal title={`${campaignDraft.id} · Recertifikácia`} onClose={() => setCampaignModal(false)} wide><div className="iam-campaign-form"><div className="iam-campaign-fields"><Field label="Názov kampane"><input value={campaignDraft.name} onChange={(event) => setCampaignDraft({ ...campaignDraft, name: event.target.value })}/></Field><Field label="Stav"><select value={campaignDraft.status} onChange={(event) => setCampaignDraft({ ...campaignDraft, status: event.target.value })}><option>Návrh</option><option>Aktívna</option><option>Ukončená</option><option>Zrušená</option></select></Field><Field label="Vlastník"><input value={campaignDraft.owner} onChange={(event) => setCampaignDraft({ ...campaignDraft, owner: event.target.value })}/></Field><Field label="Rozsah"><input value={campaignDraft.scope} onChange={(event) => setCampaignDraft({ ...campaignDraft, scope: event.target.value })}/></Field><Field label="Začiatok"><input type="date" value={campaignDraft.startDate} onChange={(event) => setCampaignDraft({ ...campaignDraft, startDate: event.target.value })}/></Field><Field label="Termín"><input type="date" value={campaignDraft.dueDate} onChange={(event) => setCampaignDraft({ ...campaignDraft, dueDate: event.target.value })}/></Field><Field label="Opis"><textarea value={campaignDraft.description} onChange={(event) => setCampaignDraft({ ...campaignDraft, description: event.target.value })}/></Field></div><div className="iam-recert-table"><table className="data-table"><thead><tr><th>Osoba</th><th>Prístup</th><th>Recenzent</th><th>Rozhodnutie</th><th>Poznámka</th></tr></thead><tbody>{campaignDraft.items.map((item) => <tr key={item.id}><td><strong>{item.subjectName}</strong><small>{item.subjectEmail}</small></td><td>{item.accessName}{item.privileged && <small className="iam-privileged">Privilegovaný</small>}</td><td><input value={item.reviewer} onChange={(event) => updateCampaignItem(item.id, { reviewer: event.target.value })}/></td><td><select value={item.decision} onChange={(event) => updateCampaignItem(item.id, { decision: event.target.value })}><option>Čaká</option><option>Ponechať</option><option>Odobrať</option><option>Upraviť</option></select></td><td><input value={item.decisionNote} onChange={(event) => updateCampaignItem(item.id, { decisionNote: event.target.value })}/></td></tr>)}</tbody></table></div></div><div className="modal-actions"><button className="button button-secondary" onClick={() => setCampaignModal(false)}>Zrušiť</button><button className="button button-primary" onClick={saveCampaign} disabled={!campaignDraft.name.trim()}>Uložiť kampaň</button></div></Modal>}
  </div>
}
