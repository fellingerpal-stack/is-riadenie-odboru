...
function normalizeProfile(row: Record<string, unknown>): UserProfile {
  return {
    ...
    role: row.role === 'admin' || row.role === 'manager' || row.role === 'resolver' || row.role === 'project_manager' || row.role === 'project_member' || row.role === 'employee' ? row.role : 'viewer',
    accessScopes: normalizeAccessScopes(row.access_scopes, row.role === 'admin' || row.role === 'manager' || row.role === 'resolver' || row.role === 'project_manager' || row.role === 'project_member' || row.role === 'employee' ? row.role : 'viewer', String(row.department ?? '')),
