...
const modules = [
  ...
  { phase: 'Fáza 5', status: 'Ďalší release', title: 'CMDB a majetok', text: 'Aplikácie, infraštruktúra, zariadenia, licencie, zmluvy, väzby služieb a dopad incidentov.', icon: 'database' as const },
]
...
export default function Roadmap({
  ...
  const canSaveCloud = configured && role !== 'viewer' && sync !== 'saving' && sync !== 'loading' && sync !== 'synced'
      ...
      <aside className="span-5 settings-side">
        {configured ? <article className="panel">
          ...
          <select value={role} onChange={(event) => onRoleChange(event.target.value as AppRole)}>
            <option value="admin">Administrátor</option><option value="manager">Manažér</option><option value="viewer">Čitateľ</option>
          </select>
          <div className="role-info"><Icon name={role === 'viewer' ? 'lock' : 'edit'} size={18}/><span>{role === 'viewer' ? 'Režim iba na čítanie.' : 'Údaje možno upravovať.'}</span></div>
        </article>}
        ...
        {configured && <article className="panel">
          ...
          <div className="stack-buttons">
            ...
            {role !== 'viewer' && <button className="button button-primary" disabled={!canSaveCloud} onClick={() => void onSaveCloud()}><Icon name="upload"/> Uložiť do databázy</button>}
          </div>
        ...
        <article className="panel">
          ...
          <div className="stack-buttons">
            ...
            <button className="button button-secondary" disabled={role === 'viewer'} onClick={() => input.current?.click()}><Icon name="upload"/> Importovať JSON</button>
            <input ref={input} hidden type="file" accept="application/json" onChange={(event) => void fileSelected(event.target.files?.[0])}/>
            <button className="button button-danger" disabled={role === 'viewer'} onClick={onReset}><Icon name="refresh"/> Obnoviť vzorové dáta</button>
          </div>
