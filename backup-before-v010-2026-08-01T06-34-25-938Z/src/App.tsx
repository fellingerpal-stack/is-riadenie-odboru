...
const navGroups:{label:string;items:NavItem[]}[]=[
  ...
  {label:'Riadenie práce',items:[{key:'capacity',label:'Kapacity',icon:'capacity'},{key:'work',label:'Projekty a úlohy',icon:'projects',badge:s=>s.tasks.filter(t=>t.status!=='Hotovo').length},{key:'helpdesk',label:'Helpdesk / ServiceDesk',icon:'helpdesk',badge:s=>(Array.isArray(s.tickets)?s.tickets:[]).filter(t=>!['Vyriešená','Uzatvorená','Zrušená'].includes(t.status)).length},{key:'changes',label:'Change management',icon:'change',badge:s=>(Array.isArray(s.changes)?s.changes:[]).filter(c=>!['Dokončená','Zamietnutá','Rollback','Zrušená'].includes(c.status)).length},{key:'problems',label:'Problem management',icon:'problem',badge:s=>(Array.isArray(s.problems)?s.problems:[]).filter(p=>!['Vyriešený','Uzatvorený'].includes(p.status)).length},{key:'iam',label:'IAM / Prístupy',icon:'iam',badge:s=>(Array.isArray(s.accessRequests)?s.accessRequests:[]).filter(r=>!['Dokončená','Zamietnutá','Zrušená'].includes(r.status)).length},{key:'cmdb',label:'CMDB / Aktíva',icon:'cmdb',badge:s=>(Array.isArray(s.cmdbItems)?s.cmdbItems:[]).filter(i=>!i.businessOwner||!i.technicalOwner||i.lifecycle==='Na obnovu').length},{key:'risks',label:'Riziká',icon:'risk',badge:s=>s.risks.filter(r=>r.status!=='Ukončené').length},{key:'decisions',label:'Rozhodnutia',icon:'decision',badge:s=>s.decisions.filter(d=>d.status!=='Schválené').length}]},
