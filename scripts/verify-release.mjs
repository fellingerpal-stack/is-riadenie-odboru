...
const expectedVersion = '0.57.0'
const errors = []

if (pkg.version !== expectedVersion) console.warn(`[v0.57.0 verify] WARNING package.json version=${pkg.version}, expected=${expectedVersion}; build continues because this is metadata only`)
...
if (errors.length) {
  console.error('[v0.57.0 verify] FAILED')
  for (const error of errors) console.error(`- ${error}`)
  process.exit(1)
}
console.log(`[v0.57.0 verify] OK; sourceFiles=${sourceFiles.length}; version=${pkg.version}`)
